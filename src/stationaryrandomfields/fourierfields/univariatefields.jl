...
"""
struct UnivariateRandomFourierField{
    N,T<:Tuple,D<:Distributions.UnivariateDistribution,Z,F<:Tuple
    ...
    zero_index_list::Z
    freq::F
    function UnivariateRandomFourierField(signaldims, dist, μ=0.0)
        dims = (rfftsize(signaldims...)..., 2)
        freq = rfftfreq(dims)
        zero_index_list = _zero_index_list(dims)
        return new{
            length(dims),typeof(dims),typeof(dist),typeof(zero_index_list),typeof(dims)
        }(
            dims, dist, eltype(dist)(mean), zero_index_list, freq
        )
    end
end

@inline function Distributions._rand!(
    rng::AbstractRNG, field::UnivariateRandomFourierField, x::AbstractArray{<:Real}
)
...
@inline function Distributions._logpdf(
    field::UnivariateRandomFourierField, x::AbstractMatrix{<:Real}
)
    # mapout logpdf
...
function UnivariateGaussianRandomFourierField(signaldims, μ=0.0)
    return UnivariateRandomFourierField(signaldims, dist::Distributions.Normal, μ)
end