...
"""
struct PowerSpectrumRandomField{N, T<:Tuple, D<:AbstractRandomFourierField, P, PI} <: AbstractPowerSpectrumRandomField{N}
    dims::T
    dist::D
    fftplan::P
    ifftplan::PI
    function PowerSpectrumRandomField(dims, dist, psdmodel, μ=0.)
        ...
        return new{length(dims), typeof(dims), typeof(fourierfield), typeof(fftplan), typeof(ifftplan)}(
            dims, fourierfield, fftplan, ifftplan
        )
    end
end

PowerSpectrumGaussianRandomField(dims, psdmodel, μ=0.) = PowerSpectrumRandomField(dims, dist::Distributions.Normal, psdmodel, μ)