...
"""
struct UnivariateRandomField{N, T<:Tuple, D<:Distributions.UnivariateDistribution} <: AbstractUnivariateRandomField{N} 
    dims::T
    dist::D
...
"""
@inline UnivariateGaussianRandomField(dims, dist::Distributions.Normal=DefaultGaussianDistribution) = UnivariateRandomField(dims, dist)
...
This type is a random field where each element is drawn from a univariate Gaussian (i.e. Normal) distribution.
"""
@inline UnivariateLaplaceRandomField(dims, dist::Distributions.Laplace=DefaultLaplaceDistribution) = UnivariateRandomField(dims, dist)